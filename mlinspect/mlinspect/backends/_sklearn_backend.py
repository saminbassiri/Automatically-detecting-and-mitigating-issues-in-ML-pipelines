...
from ._backend_utils import get_csr_row_iterator
...
class SklearnBackend(Backend):
    ...
    def before_call(operator_context, input_infos: List[AnnotatedDfObject]):
        ...
        if operator_context.operator in [OperatorType.ESTIMATOR, OperatorType.PREDICT]:
            if isinstance(input_infos[0], csr_matrix):
                input_infos[0].result_annotation = get_csr_row_iterator(input_infos[0])[0]
        return input_infos
    ...
    def after_call(operator_context, input_infos: List[AnnotatedDfObject], return_value,
        ...
        if operator_context.operator == OperatorType.DATA_SOURCE:
            ...
            if len(return_value) == 2:
                ...
                train_data, test_data, y_train, y_test = return_value
            else:
                raise ValueError(f"Unexpected number of return values: {len(return_value)}")
            ...
            return_value = execute_inspection_visits_nary_op(operator_context, input_infos, return_value,
                                                             non_data_function_args)
        elif operator_context.operator == OperatorType.PIPELINE:
            return_value = execute_inspection_visits_nary_op(operator_context, [input_infos], return_value,
                                                             non_data_function_args)
        else:
