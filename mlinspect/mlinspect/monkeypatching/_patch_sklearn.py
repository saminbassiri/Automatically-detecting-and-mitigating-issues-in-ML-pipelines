...
class SklearnPipelinePatching:
    ...
    @gorilla.name('__init__')
    @gorilla.settings(allow_hit=True)
    def patched__init__(self, steps, *,  memory = None, verbose = False):
        print(""" Patch for ('sklearn.pipeline.__init__', 'Pipeline') """)
        original = gorilla.get_original_attribute(pipeline.Pipeline, '__init__')

        def execute_inspections(_, caller_filename, lineno, optional_code_reference, optional_source_code):
            """ Execute inspections, add DAG node """
            original(self, steps, memory=memory, verbose=verbose)
    ...
    @gorilla.settings(allow_hit=True)
    def patched_fit_transform(self, *args, **kwargs):
        print(""" Patch for ('sklearn.pipeline.fit_transform', 'Pipeline') """)
        call_info_singleton.transformer_filename = self.mlinspect_filename
        call_info_singleton.transformer_lineno = self.mlinspect_lineno
        call_info_singleton.transformer_function_info = FunctionInfo('sklearn.pipeline.fit_transform', 'Pipeline.fit_transform')
        call_info_singleton.transformer_optional_code_reference = self.mlinspect_optional_code_reference
        call_info_singleton.transformer_optional_source_code = self.mlinspect_optional_source_code
        ...
        input_tuple = args[0]
        input_infos = get_input_info(input_tuple, self.mlinspect_filename, self.mlinspect_lineno, call_info_singleton.transformer_function_info,
                                        self.mlinspect_optional_code_reference, self.mlinspect_optional_source_code)
        
        operator_context = OperatorContext(OperatorType.PIPELINE, call_info_singleton.transformer_function_info)
        input_annotated_dfs = input_infos.annotated_dfobject
        backend_input_infos = SklearnBackend.before_call(operator_context, input_annotated_dfs)
        result = original(self, *args, **kwargs)
        backend_result = SklearnBackend.after_call(operator_context,
                                                   backend_input_infos,
        ...
        dag_node = DagNode(singleton.get_next_op_id(),
                           ...
                           get_optional_code_info_or_none(self.mlinspect_optional_code_reference,
                                                          self.mlinspect_optional_source_code))
        input_dag_nodes = input_infos.dag_node
        add_dag_node(dag_node, [input_dag_nodes], backend_result)
    ...
    @gorilla.settings(allow_hit=True)
    def patched_transform(self, *args, **kwargs):
        print(""" Patch for ('sklearn.pipeline.transform', 'Pipeline') """)
        call_info_singleton.pipeline_filename = self.mlinspect_filename
        call_info_singleton.pipeline_lineno = self.mlinspect_lineno
        call_info_singleton.pipeline_function_info = FunctionInfo('sklearn.pipeline.transform', 'Pipeline.transform')
        call_info_singleton.pipeline_optional_code_reference = self.mlinspect_optional_code_reference
        call_info_singleton.pipeline_optional_source_code = self.mlinspect_optional_source_code

        original = gorilla.get_original_attribute(pipeline.Pipeline, 'transform')
        input_infos = args[0]
        function_info = FunctionInfo('sklearn.pipeline.Pipeline.transform', 'Pipeline.transform')
        input_infos = get_input_info(input_infos, self.mlinspect_filename, self.mlinspect_lineno, function_info,
                                        self.mlinspect_optional_code_reference, self.mlinspect_optional_source_code)
        
        operator_context = OperatorContext(OperatorType.PIPELINE, function_info)
        input_annotated_dfs = input_infos.annotated_dfobject
        backend_input_infos = SklearnBackend.before_call(operator_context, input_annotated_dfs)
        result = original(self, *args, **kwargs)
        backend_result = SklearnBackend.after_call(operator_context,
...
@gorilla.patches(preprocessing.Normalizer)
class SklearnNormalizerPatching:
    """ Patches for sklearn Normalizer """

    @gorilla.name('__init__')
    @gorilla.settings(allow_hit=True)
    def patched__init__(self, *, norm='l2',
                        mlinspect_caller_filename=None, mlinspect_lineno=None,
                        mlinspect_optional_code_reference=None, mlinspect_optional_source_code=None,
                        mlinspect_fit_transform_active=False):
        """ Patch for ('sklearn.preprocessing._data', 'Normalizer') """
        original = gorilla.get_original_attribute(preprocessing.Normalizer, '__init__')
        ...
        self.mlinspect_non_data_func_args = {'norm': norm}
    ...
    @gorilla.settings(allow_hit=True)
    def patched_fit_transform(self, *args, **kwargs):
        """ Patch for ('sklearn.preprocessing._data.Normalizer', 'fit_transform') """
        self.mlinspect_fit_transform_active = True  
        original = gorilla.get_original_attribute(preprocessing.Normalizer, 'fit_transform')
        function_info = FunctionInfo('sklearn.preprocessing._data', 'Normalizer')
        input_info = get_input_info(args[0], self.mlinspect_caller_filename, self.mlinspect_lineno, function_info,
                                    self.mlinspect_optional_code_reference, self.mlinspect_optional_source_code)
        ...
        dag_node = DagNode(singleton.get_next_op_id(),
                           BasicCodeLocation(self.mlinspect_caller_filename, self.mlinspect_lineno),
                           operator_context,
                           DagNodeDetails("Normalizer: fit_transform", ['array']),
                           get_optional_code_info_or_none(self.mlinspect_optional_code_reference,
                                                          self.mlinspect_optional_source_code))
        add_dag_node(dag_node, [input_info.dag_node], backend_result)
    ...
    @gorilla.settings(allow_hit=True)
    def patched_transform(self, *args, **kwargs):
        """ Patch for ('sklearn.preprocessing._data.Normalizer', 'transform') """
        original = gorilla.get_original_attribute(preprocessing.Normalizer, 'transform')
        if not self.mlinspect_fit_transform_active:
            function_info = FunctionInfo('sklearn.preprocessing._data', 'Normalizer')
            input_info = get_input_info(args[0], self.mlinspect_caller_filename, self.mlinspect_lineno, function_info,
                                        self.mlinspect_optional_code_reference, self.mlinspect_optional_source_code)
            ...
            dag_node = DagNode(singleton.get_next_op_id(),
                               BasicCodeLocation(self.mlinspect_caller_filename, self.mlinspect_lineno),
                               operator_context,
                               DagNodeDetails("Normalizer: transform", ['array']),
                               get_optional_code_info_or_none(self.mlinspect_optional_code_reference,
                                                              self.mlinspect_optional_source_code))
            add_dag_node(dag_node, [input_info.dag_node], backend_result)
        else:
...
class SklearnFunctionTransformerPatching:
    ...
    def patched__init__(self, func=None, inverse_func=None, *, validate=False, accept_sparse=False, check_inverse=True,
                        kw_args=None, inv_kw_args=None, feature_names_out=None, mlinspect_caller_filename=None, mlinspect_lineno=None,
                        mlinspect_optional_code_reference=None, mlinspect_optional_source_code=None,
                        mlinspect_fit_transform_active=False):
        ...
        self.mlinspect_non_data_func_args = {'validate': validate, 'accept_sparse': accept_sparse,
                                             ...
                                             'inv_kw_args': inv_kw_args, "feature_names_out": feature_names_out}
...
class SklearnDecisionTreePatching:
    ...
    def patched_fit(self, *args, **kwargs):
        ...
        global Prediction_added
        if Prediction_added:
            original(self, *args, **kwargs)
            return self  # Skip adding to DAG if already added
        
        if not call_info_singleton.param_search_active:
            function_info = FunctionInfo('sklearn.tree._classes', 'DecisionTreeClassifier.fit')
    ...
    def patched_predict_proba(self, *args, **kwargs):
        ...
        original = gorilla.get_original_attribute(tree.DecisionTreeClassifier, 'predict_proba')
        
        global classifier_added
        if classifier_added:
            prediction_result = original(self, *args, **kwargs)
            return prediction_result  
        
        if not call_info_singleton.param_search_active:
            print("predict_proba DecisionTreeClassifier")
            
            function_info = FunctionInfo('sklearn.tree._classes', 'DecisionTreeClassifier.predict')
            
            data_backend_result, predict_data_node, predict_data_result = add_train_data_node(self, args[0], function_info)

            operator_context = OperatorContext(OperatorType.PREDICT, function_info)
            input_dfs = [data_backend_result.annotated_dfobject]
            input_infos = SklearnBackend.before_call(operator_context, input_dfs)

            prediction_result = original(self, predict_data_result, *args[1:], **kwargs)

            backend_result = SklearnBackend.after_call(operator_context,
                                                                input_infos,
                                                                prediction_result,
                                                                self.mlinspect_non_data_func_args)
            predict_backend_result = BackendResult(backend_result.annotated_dfobject,
                                                 backend_result.dag_node_annotation)
            self.mlinspect_predict_node_id = singleton.get_next_op_id()
            dag_node = DagNode(self.mlinspect_predict_node_id,
                            BasicCodeLocation(self.mlinspect_caller_filename, self.mlinspect_lineno),
                            operator_context,
                            DagNodeDetails("Decision Tree Predict", []),
                            get_optional_code_info_or_none(self.mlinspect_optional_code_reference,
                                                            self.mlinspect_optional_source_code))
            add_dag_node(dag_node, [predict_data_node], predict_backend_result)
            classifier_added = True
        else:
            prediction_result = original(self, *args, **kwargs)
...
class SklearnLogisticRegressionPatching:
    ...
    @gorilla.settings(allow_hit=True)
    def patched_predict_proba(self, *args, **kwargs):
        """ Patch for ('sklearn.tree._classes.LogisticRegression', 'predict_proba') """
        original = gorilla.get_original_attribute(linear_model.LogisticRegression, 'predict_proba')
        
        
        if not call_info_singleton.param_search_active:
            print("predict_proba LogisticRegression")
            
            function_info = FunctionInfo('sklearn.tree._classes', 'LogisticRegression.predict')
            ...
            dag_node = DagNode(self.mlinspect_predict_node_id,
                            BasicCodeLocation(self.mlinspect_caller_filename, self.mlinspect_lineno),
                            operator_context,
                            DagNodeDetails("Logistic Regression Predict", []),
                            get_optional_code_info_or_none(self.mlinspect_optional_code_reference,
                                                            self.mlinspect_optional_source_code))
            add_dag_node(dag_node, [predict_data_node], predict_backend_result)
        else:
