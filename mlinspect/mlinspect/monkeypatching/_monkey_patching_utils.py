...
def get_input_info(df_object, caller_filename, lineno, function_info, optional_code_reference, optional_source_code) \
    ...
    if hasattr(df_object, "_mlinspect_annotation"):
        ...
        if optional_code_reference:
            ...
            code_reference = f"({optional_source_code})"
        description = f"Warning! Operator {lineno} {code_reference} encountered a " \
...
def get_column_names(df_object):
    ...
    elif isinstance(df_object, (csr_matrix, csc_matrix, numpy.ndarray)):
        columns = ['array']
    elif isinstance(df_object, str):
        columns = [df_object]
    else:
...
def get_dag_node_for_id(dag_node_id: int):
    ...
    """
    # pylint: disable=protected-access
    return singleton.op_id_to_dag_node.get(dag_node_id)
...
def get_optional_code_info_or_none(optional_code_reference: CodeReference or None,
    ...
    """
    if singleton.track_code_references:
        if optional_code_reference is None or optional_source_code is None:
            # Log a warning instead of asserting (to avoid crashes)
            print("⚠️ Warning: Expected code reference but found None. This may affect DAG tracking.")
            return None  # Return None instead of failing
        return OptionalCodeInfo(optional_code_reference, optional_source_code)
    
    # If tracking is disabled, ensure both values are None
    if optional_code_reference is not None or optional_source_code is not None:
        print("⚠️ Warning: Unexpected code reference values when tracking is disabled.")
...
def add_test_data_node(estimator, train_data_arg, function_info):
    """Add a Test Data DAG Node for a estimator.predict call"""
    input_info_train_data = get_input_info(train_data_arg, estimator.mlinspect_caller_filename,
                                           estimator.mlinspect_lineno, function_info,
                                           estimator.mlinspect_optional_code_reference,
                                           estimator.mlinspect_optional_source_code)
